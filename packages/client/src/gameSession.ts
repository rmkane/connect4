...
export function mountGameSession(opts: {
  ...
  function gamePicker(isTableHost: boolean, showLastRecap: boolean): TemplateResult {
    ...
    const recapBtn = showLastRecap
      ? html`
          <button
            type="button"
            class="w-full rounded-xl border border-violet-200 bg-violet-50/90 px-4 py-3 text-left text-sm font-semibold text-violet-900 shadow-sm transition hover:border-violet-300 hover:bg-violet-100/90 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 focus-visible:ring-offset-2"
            @click=${openGameRecap}
          >
            <span class="block text-xs font-normal text-violet-700/90">Timing & turns</span>
    ...
    const gameCard = (
      ...
      <button
        type="button"
        class="group ${opts.accent} relative flex min-h-34 flex-col overflow-hidden rounded-2xl border-2 bg-white p-4 text-left shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
        @click=${() => pick(kind)}
      >
        <div
          class="pointer-events-none absolute -top-2 -right-2 text-5xl opacity-[0.12] transition select-none group-hover:opacity-[0.18]"
          aria-hidden="true"
        >
          ${opts.icon}
        </div>
        <div class="relative flex flex-1 flex-col">
          <div
            class="mb-3 flex h-11 w-full items-center justify-center rounded-lg bg-zinc-50/90 ring-1 ring-zinc-100"
          >
            ${opts.art}
          </div>
          <span class="text-base font-bold tracking-tight text-zinc-900">${opts.title}</span>
          <span class="mt-1.5 text-xs leading-snug text-zinc-600">${opts.blurb}</span>
          <span
            class="mt-auto pt-3 text-[11px] font-semibold tracking-wider text-zinc-400 uppercase transition group-hover:text-zinc-600"
            >Play →</span
          >
        </div>
      </button>
    ...
    return html`
      ...
      >
        <div class="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 id="pick-game-heading" class="text-xl font-bold tracking-tight text-zinc-900">
              Pick a table game
            </h2>
            <p class="mt-1 max-w-md text-sm text-zinc-600">
              ${isTableHost
                ? "You're the host — choose a title. You can switch after each match."
                : 'Hang tight — the host is choosing what to play.'}
            </p>
          </div>
          <span
            class="hidden rounded-full bg-zinc-900/5 px-3 py-1 text-[11px] font-semibold tracking-wide text-zinc-500 uppercase sm:inline"
            aria-hidden="true"
            >Lobby</span
          >
        </div>
        ...
        ${isTableHost
          ...
          : html`
              <div
                class="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50/80 px-4 py-8 text-center"
                role="status"
              >
